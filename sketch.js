...
function setup() {
  ...
  bottle = new Bottle(Math.round(random(500, 1100)), 200, 200, 300)
...
function keyPressed() {
  if (keyCode === DOWN_ARROW) {
    var bullet = new Bullet(200, 266, 90, 90);
...
function showBullets(bullet, index) {
  ...
  if (bullet.body.position.x >= width || bullet.body.position.y >= 530) {
...
function showbottles() {
  ...
  if (bottles.length > 0) {
    for (var i = 0; bottles.length > i; i++) {
      bottles[i].display();
    }
  } else {
    bottle = new Bottle(Math.round(random(500, 1100)), 350, 100, 200)
    bottles.push(bottle)
  }
}